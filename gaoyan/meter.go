...
func (m METER) Read(host string, port int) (json_string string, err error) {
	p := modbus.NewTCPClientProvider(fmt.Sprintf("%s:%d", host, port), modbus.WithEnableLogger()) //|modbus.WithTCPTimeout(3000)
	...
	if cerr != nil {
		...
		results, err := client.ReadHoldingRegisters(0x01, 0x00, length)
